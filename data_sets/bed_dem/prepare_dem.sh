...
infile=$1
ver=$2
for grid in 16000 8000 4000 2000 1000; do
    ...
    if [ ! -f map_weights_${grid}m.nc ]; then
        echo "Preparing weights for ${grid}m grid"
        cdo -v -f nc4 -z zip_2 -P $NN genycon,../grids/g${grid}m.txt bedmap2_bed.nc map_weights_${grid}m.nc
    fi
    if [ ! -f pen_weights_${grid}m.nc ]; then
        echo "Preparing weights for ${grid}m grid"
        cdo -v -f nc4 -z zip_2 -P $NN genycon,../grids/g${grid}m.txt bedrock.nc pen_weights_${grid}m.nc
    fi
done
...
for grid in 16000 8000 4000 2000 1000 500; do
    ...
    ncks -O -4 -d x,-2711000.,-1760000. -d y,800000.,1760000. $outfile pism_BedMachinePeninsula_v$ver_g${grid}m.nc
    ...
    cp pism_BedMachineAntarctica_v$ver_g${grid}m.nc pism_BedMachineHFAntarctica_v$ver_g${grid}m.nc
    ncks -A -v bed_hf  pism_HFAntarctica_g${grid}m.nc  pism_BedMachineHFAntarctica_v$ver_g${grid}m.nc
    ncap2 -O -s "where(bed_hf!=-9999) {bed=bed_hf; thickness=(surface-bed_hf);}; where(thickness<0) {thickness=0;};" pism_BedMachineHFAntarctica_v$ver_g${grid}m.nc pism_BedMachineHFAntarctica_v$ver_g${grid}m.nc
    ncks -O -v bed_hf -x  pism_BedMachineHFAntarctica_v$ver_g${grid}m.nc  pism_BedMachineHFAntarctica_v$ver_g${grid}m.nc
    cp pism_BedMachineHFAntarctica_v$ver_g${grid}m.nc  pism_BedMachineBedmapHFAntarctica_v$ver_g${grid}m.nc
    cdo -f nc4 -z zip_2 -P $NN remap,../grids/g${grid}m.txt,map_weights_${grid}m.nc bedmap2_bed.nc bedmap2_bed_g${grid}m.nc
    ncks -A -v Band1  bedmap2_bed_g${grid}m.nc pism_BedMachineBedmapHFAntarctica_v$ver_g${grid}m.nc
    ncatted -a _FillValue,Band1,d,, -a missing_value,Band1,d,, pism_BedMachineBedmapHFAntarctica_v$ver_g${grid}m.nc
    ncap2 -O -s "where((mask==0) && (Band1<30000.0)) bed=Band1; where(mask==4) {bed=surface-thickness;};" pism_BedMachineBedmapHFAntarctica_v$ver_g${grid}m.nc pism_BedMachineBedmapHFAntarctica_v$ver_g${grid}m.nc
    ncks -O -v Band1 -x pism_BedMachineBedmapHFAntarctica_v$ver_g${grid}m.nc pism_BedMachineBedmapHFAntarctica_v$ver_g${grid}m.nc
done
